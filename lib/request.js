 ...
 */
exports = module.exports = class Request extends Emitter {
    /**
    ...
    get cache() {
        ...
        if ((status >= 200 && status < 300) || 304 == status) {
            ...
            let IfModifiedSince = this.get('If-Modified-Since');
            let IfNoneMatch = this.get('If-None-Match');
            let CacheControl = this.get('Cache-Control');
            CacheControl = CacheControl.toLocaleLowerCase();

            let LastModified = this.response.get('Last-Modified');
            ...
            if (IfNoneMatch) {
                // parse if-none-match
                IfNoneMatch = IfNoneMatch.split(/ *, */);
                EtagMatches = IfNoneMatch.some(function (match) {
            ...
            if (IfModifiedSince) {
                ...
                NotModified = LastModified <= IfModifiedSince;
            }
